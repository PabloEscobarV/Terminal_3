...
/*   Updated: 2025/03/26 01:02:40 by Pablo Escob      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_uint	set_in_out_file(t_llist **data, t_argv *argvt)
{
	t_uint	llist_size;
	t_llist **first_node;
	...
	{
		data = (*data)->next;
		if (set_file(get_args(*data)->data, get_args((*data)->previous)->operation, argvt))
		{
			get_args(*data)->data = NULL;
			continue ;
		}
		if (get_args((*data)->previous)->data && *(get_args((*data)->previous)->data))
			++llist_size;
		if (get_args(*data)->operation >= 0)
			break ;
	}
	data = first_node;
	return (llist_size);
...
static char	**get_strv_from_llst(t_llist **llst, t_uint llist_size)
{
	t_uint	i;
	char		**strv;
	...
	{
		if (get_args(*llst)->data && *((get_args)(*llst)->data))
		{
			strv[i] = (*llst)->data;
			++i;
		}
...
static t_argv	*get_argvt(t_llist **data)
{
	...
	argvt = crt_argvt();
	llist_size = set_in_out_file(*data, argvt);
	if (!llist_size && get_args((*data)->previous)->data && *(get_args((*data)->previous)->data))
		llist_size = 1;
	argvt->argv = get_strv_from_llst(*data, llist_size);
	argvt->operation = get_args((*data)->previous)->operation;
